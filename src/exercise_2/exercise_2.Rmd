...
You can use this implementation for the exercises

```{julia, results = 'hide'}
"""
# Basic Metropolis Markov Chain sampler

### Arguments

- `lp`: A function that the log density (up to a constant). Takes only a single argument.

- `θinit`: Inital value. Check if `lp(θinit)` gives a valid result.

- `nsamples`: length of the Markov chain

- `Σjump`: covariance matrix of the multivariate normal jump distribution

### Value

A names tuple with fields:

- `chain`: the markov chain

- `lp`: the corresponding log densities
...
Σ = diagm(ones(3))
res = metropolis_MCMC(logposterior_monod, par; nsampler=1000, Σjump = Σ);
...
Array(chn)                      # convert it to an normal Array
```
...
- Try to manualy change the variance of the jump distribution. Can we
  get better chains?

- Use the previous chain and estimate it's covariance. Does a
...
If you use the function `metropolis_MCMC` as defined above, you can
pass the covariance matrix of the jump
distribution with the argument `Σjump`.
