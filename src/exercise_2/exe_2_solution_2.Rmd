...
```{python}
# If models.py are not stored in the same directory as the scripts, one should 
...
# Now, remove the directory from the Python path to avoid conflicts
...
def logposterior(par):
    return logposterior_monod(par)

# Initialize the `emcee` sampler
sampler = emcee.EnsembleSampler(nwalkers, ndim, logposterior, moves=move)

# Run the MCMC chain
sampler.run_mcmc(init_pos, n_samples, progress=True)
...
In emcee, the default move type used in the EnsembleSampler is the stretch move 
(StretchMove). This move is well-suited for a wide range of models and performs 
efficiently in high-dimensional parameter spaces. The stretch move works by 
randomly choosing a pair of walkers and proposing a new position for one walker
based on a stretch factor relative to the other walker’s position.
