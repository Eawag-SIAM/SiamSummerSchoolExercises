...
-  the determinate of the Jacobin of this functions.
...
# make sure that we can run both out function for all values in ℝ^n
par_test = randn(3)
...
[`AdvancedHMC`](https://github.com/TuringLang/AdvancedHMC.jl) provides
 building blocks that can be combined to various HMC samplers. The
...
proposed a simple MCMC algorithm that uses the gradient to adapt the
jump distribution. The package
