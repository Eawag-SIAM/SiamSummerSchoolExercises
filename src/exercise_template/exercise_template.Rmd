---
title: R Markdown Template for Summer School Exercises
...
`R Markdown` files (typical file ending is `*.rmd`) are compiled with
...
<!-- include the solutions if `showsolutions==TRUE` -->
```{r, child = if(params$showsolutions){"exercise_template_solution_1.Rmd"}}
```

# Math

LaTeX syntax is used for equations. Either inline $a=\int g(x)\, \text{d}x$ or as
...
[This online book](https://bookdown.org/yihui/rmarkdown/) provides a very comprehensive description of what can be done with R Markdown.
