...
#### Julia
Use the package `Distributions.jl` to have access to many probability distributions, like a uniform or a gaussian one. Type `?Distributions` in the Julia REPL to get information about this package. You can sample n random numbers following a certain distribution `d_X` via the command `rand(d_X, n)`.

Here are some useful functions: for any distribution `__`
```{julia, eval = FALSE}
pdf(__, x)       #evaluate pdf at x
cdf(__, x)       #evaluate cdf at x
quantile(__, p)  #evaluate the p-th quantile
rand(__, n)      #sample n random numbers
```
...
#### Julia
Given a mean vector $\mu$ and a correlation matrix $\Sigma$, you can construct a multivariate distribution, using the function `MvNormal(\mu, \Sigma)` from the package `Distribution.jl`.
...
#### Julia
You can use the function `iqr` from the `Statsbase.jl`package, as well as the `percentile`function.

To plot histograms, you can use the package `Plots.jl`as already mentionned. You can also scatter from this package, using the function `scatter`. However, to plot densities, you have to use the function `density`from the package `StatsPlots.jl`.

Use the package `Statistics.jl`to find the covariance and correlation matrix.
...
#### Julia
Use the packages `CSV.jl` (Comma Separated Values) and `DataFrames.jl` to read and manipulate data. Use the function `pwd()`the see which directory you are currently working in, and read the desired file using a relative path. You can use the function `joinpath`to make the process easier. 
...
#### Julia
Use `LogNormal()` from the package `Distributions.jl`for a log-normal distribution. You can create you brand new Julia function following the syntax
```{julia, eval = FALSE}
function choose_your_name(arg_1, arg_2)
    return arg_1 + arg_2 #for example
end
```
