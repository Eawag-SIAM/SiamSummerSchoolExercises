...
interquartiles_indep = [iqr(Y_indep_sample[1, :]),
                        iqr(Y_indep_sample[2, :])]

interquartiles_dep = [iqr(Y_dep_sample[1, :]),
                      iqr(Y_dep_sample[2, :])]

inter_5_95_quantile_indep = [diff(quantile(Y_indep_sample[1, :], [0.05, 0.95])),
                             diff(quantile(Y_indep_sample[2, :], [0.05, 0.95]))]

inter_5_95_quantile_dep = [diff(quantile(Y_dep_sample[1, :], [0.05, 0.95])),
                           diff(quantile(Y_dep_sample[2, :], [0.05, 0.95]))]
```
...
Note, we could also have written a loop for the four plots.
