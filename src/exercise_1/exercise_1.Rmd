...
* Understand the concept of a likelihood function. In particular, be
  able to write down the likelihood functions for our toy models and
  to be able to write code for the evaluation of these likelihood
  functions.
* Perform a maximum likelihood estimation.
* Understand the difference between evaluating a likelihood function and sampling
...
Read the data provided in the file `model_linear.dat` and use them as "observation data" to compute the log-likelihood for two parameter sets: $\{\beta = 2,\, \gamma = 1, \, \sigma = 1\}$  and $\{ \beta = 3,\, \gamma = 2, \, \sigma = 1 \}$.
...
Produce deterministic model outputs of the model "Monod" over a
concentration ($C$) range from 0 to 10 with the default parameter values $r_{\mathrm{max}} = 5$ and $K = 3$.

Also produce deterministic model outputs of the model "Growth" over a time
interval from 0 to 2 with default parameter values
...
You can find the models implemented as `model.monod` and
...
(i.e. samples from the probabilistic model), given by the
deterministic part plus normal i.i.d. noise. Use $r_{\mathrm{max}} =
5$, $K = 3, and $\sigma = 0.2$

Use this function to simulate several thousand probabilistic model
realisations, for fixed model parameters, and plot the 10% and 90% quantiles as continuous prediction bands.
...
# 4. Likelihood and forward fimulation for the model "Survival"
...
For the models "Monod" and "Growth", do the following exercises:


## Manual sensitivity analysis

Increase the model parameters sequentially by $10 \%$ and by $50 \%$, redo the forward simulations, and try to understand the effect of the parameter change on the model results.


## Local sensitivity analysis

Do local sensitivity analysis (see Equation (7.3)) with uncertainty ranges of $10 \%$ and by $50 \%$ of the parameter values. Interpret the results.


## Variance-based sensitivity
Do variance-based regional sensitivity analyses (see Equation (7.19)) with lognormal parameter distributions based on default mean values and standard deviations of $10 \%$ and $50 \%$ of the mean and interpret the results.

Use the FAST sampling as implemented in the package `sensitivity` and then by generating a random sampling (mimicking a situation in which a sample would already be available).

## Comparing different sensitivity measures

Plot $s_{i,j}^{\mathrm{loc}}$ (Equation (7.3)) and $s_{i,j}^{\mathrm{var}}$
(Equation (7.19)).

Interpret the differences between the results of the local and regional analyses for both standard deviations.
