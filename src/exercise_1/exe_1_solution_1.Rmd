...
### Julia
Implementing the negative loglikelihood that we will minimize.
```{julia, results = 'hide'}
beta_init = 2
gamma_init = 1
sigma_init = 1
...
```{julia}
using Optim
param = Optim.minimizer(optimize(neg_loglikelihood_linear, [2.0, 1.0, log(0.5)]));
best_param = [param[1], param[2], exp(param[3])]
```
...
```
```{julia}
linear_mod = GLM.lm(fm, linear_data)
```
