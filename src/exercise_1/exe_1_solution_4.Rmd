...
* t, a vector of the form 0:k, if we want k+1 bins,
...
pred_survival = [survival(N, t, lambda) for _ in 1:n_sample]
```
Plotting
```{julia}
...
function loglikelihood_survival(N::Int64, t, y::Vector, lambda)
    S = exp.(-lambda .* t) ; S = push!(S, 0)
    p = .-diff(S)
    return logpdf(Multinomial(N, p), y)
end
